				...
				<div class="table-container desktop_overflow_none" style="padding:15px;">
					<table id="tabel_laporan_pemasukan" class="table table-hover table-striped table-bordered">
						...
						<?php
							...
							$bulan=date('m');
							$tahun=date('Y');
							$data = mysqli_query($koneksi,"select * from laporan_pemasukan where no_resi LIKE 'BTS%' AND month(tanggal)>='$bulan' AND year(tanggal)>='$tahun'");
						...
						<?php
							...
							$bulan=date('m');
							$tahun=date('Y');
							$data = mysqli_query($koneksi,"select * from laporan_pemasukan where no_resi LIKE 'DBT%' AND month(tanggal)>='$bulan' AND year(tanggal)>='$tahun'");
							while($d = mysqli_fetch_array($data)){
						?>	
						<tr>
							<td style="text-align:center; width:5px;">
